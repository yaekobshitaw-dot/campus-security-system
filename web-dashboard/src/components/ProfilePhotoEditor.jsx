...
const ProfilePhotoEditor = ({ user, onUpdated }) => {
  ...
  const upload = async () => {
    ...
    try {
      const response = await api.put(`/users/${user.user_id}/profile-photo`, formData);
  ...
  const remove = async () => {
    ...
    setMessage('');
    try {
      const response = await api.delete(`/users/${user.user_id}/profile-photo`);
      if (!response.data?.success || response.data?.data?.profile_photo_url) {
  ...
  return (
    <div className="mt-5 border-t border-slate-200 pt-5">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" className="dashboard-button" onClick={() => inputRef.current?.click()} disabled={loading}>Choose image</button>
      ...
      {previewUrl && <div className="mt-4 flex flex-wrap items-center gap-4"><img src={previewUrl} alt="Selected profile preview" className="h-24 w-24 rounded-xl object-cover ring-2 ring-emerald-200" /><button type="button" className="dashboard-button primary" onClick={upload} disabled={loading}>{loading ? 'Saving...' : 'Upload photo'}</button></div>}
